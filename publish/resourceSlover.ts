...
export function solveResourceWithDict(inputDir: string, outputDir: string, dict: { [url: string]: ResInfo }, version: number) {
    if (dict) {
        ...
        if (dict) {
            ...
            writeFile(resArr, outputDir, version);
...
function writeFile(resArr: ResMd5Data[], outputDir: string, version: number) {
    if (resArr && resArr.length > 0) {
        let writeStr = "";
        for (let i = 0; i < resArr.length; i++) {
            let info = resArr[i];
            if (info) {
                let { md5File, uri } = info;
...
export async function copyResToFile(resArr: ResMd5Data[], outputDir: string) {
    if (resArr) {
